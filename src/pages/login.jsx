import { useAuth } from '@/contexts/AuthContext';
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { Loader2, Sparkles } from "lucide-react";

const LoginPage = () => {
  const { login, loading, user } = useAuth();
  const [isLoggingIn, setIsLoggingIn] = useState(false);
  const navigate = useNavigate();



  const handleGoogleLogin = async () => {
    try {
      setIsLoggingIn(true);
      await login();
      // Login berhasil, redirect akan ditangani oleh useEffect di atas
    } catch (error) {
      console.error('Login error:', error);
      alert('Login gagal. Silakan coba lagi.');
    } finally {
      setIsLoggingIn(false);
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center supabase-gradient-dark">
        <div className="w-full max-w-md mx-auto p-8">
          <div className="text-center space-y-4">
            <Loader2 className="h-8 w-8 animate-spin mx-auto text-green-400" />
            <p className="text-gray-400">Memeriksa status autentikasi...</p>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen flex items-center justify-center supabase-gradient-dark p-4">
      <div className="w-full max-w-md mx-auto p-8">
        <div className="text-center space-y-2 mb-6">
          <div className="flex items-center justify-center space-x-2">
            <div className="w-8 h-8 supabase-gradient rounded-lg flex items-center justify-center">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            <span className="text-2xl text-white font-bold">Threadify.ai</span>
          </div>
          <div className="text-gray-400">
            Silakan login untuk melanjutkan
          </div>
        </div>
        <div className="space-y-6">
          <Button
            onClick={handleGoogleLogin}
            disabled={isLoggingIn}
            className="w-full supabase-gradient hover:opacity-90 text-white font-semibold"
            size="lg"
          >
            {isLoggingIn ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Memproses...
              </>
            ) : (
              <>
                <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="currentColor"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  />
                </svg>
                Login dengan Google
              </>
            )}
          </Button>
          
          <Separator className="bg-white/10" />
          
          <div className="text-center space-y-2">
            <p className="text-xs text-gray-400">
              Dengan login, Anda menyetujui{' '}
              <Button variant="link" className="h-auto p-0 text-xs text-green-400 hover:text-green-300">
                Syarat dan Ketentuan
              </Button>{' '}
              serta{' '}
              <Button variant="link" className="h-auto p-0 text-xs text-green-400 hover:text-green-300">
                Kebijakan Privasi
              </Button>
            </p>
            <Badge variant="outline" className="text-xs bg-black/20 text-gray-400 border-white/10">
              Versi Beta
            </Badge>
          </div>
        </div>
      </div>
    </div>
  );
};

export default LoginPage;